...
- [ ] Интеграция с API (CRUD меток, фильтрация маршрутов).
...
### ⚙️ Backend
# Backend Task — CRUD API для Marks (FastAPI)

## Цель

Реализовать REST API для работы с пользовательскими метками (**marks**) на карте. Сущность `mark` представляет собой точку на карте с описанием, координатами и опциональными тегами.  

API должно предоставлять базовые CRUD-операции:

1. `GET /marks` — получить все метки  
2. `GET /marks/{mark_id}` — получить метку по ID  
3. `POST /marks` — создать новую метку  
4. `PUT /marks/{mark_id}` — обновить существующую метку  
...

{
  "example_GET_/marks_response": [
    {
      "id": 1,
      "text": "Уютное кафе с розетками и Wi-Fi",
      "tags": ["кафе", "wi-fi", "розетки"],
      "location": {
        "latitude": 59.9386,
        "longitude": 30.3141
      },
      "created_at": "2025-10-04T10:15:30Z",
      "updated_at": "2025-10-04T10:15:30Z"
    },
    {
      "id": 2,
      "text": "Парковка рядом с вокзалом",
      "tags": ["парковка", "транспорт"],
      "location": {
        "latitude": 59.9350,
        "longitude": 30.3200
      },
  ...
  "example_GET_/marks_{mark_id}_response": {
    "id": 1,
    "text": "Уютное кафе с розетками и Wi-Fi",
    "tags": ["кафе", "wi-fi", "розетки"],
    "location": {
  ...
  "example_POST_/marks_request": {
    "text": "Хорошее место для прогулки с собакой",
    "location": {
      "latitude": 59.9400,
      "longitude": 30.3200
    }
  ...
  "example_POST_/marks_response": 201 Created,

  "example_PUT_/marks_{mark_id}_request": {
    "text": "Обновлённое описание места",
    "tags": ["отдых", "парк"],
    "location": {
      "latitude": 59.9410,
      "longitude": 30.3210
  ...
  "example_PUT_/marks_{mark_id}_response": {
    PATH "id": 3,
    "text": "Обновлённое описание места",
    "tags": ["отдых", "парк"],
    "location": {
